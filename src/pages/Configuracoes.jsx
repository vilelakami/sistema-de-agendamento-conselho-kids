...
function Configuracoes({ abrirModal }) {
  ...
  const salvarEdicaoUsuario = (index) => {
    ...
    if (container) {
      ...
      const novaSenha = container.querySelectorAll('input[type="password"]')[0]
        .value;
      const confirmarSenha = container.querySelectorAll(
        'input[type="password"]',
      )[1].value;
  ...
  const salvarDadosAdmin = () => {
    const dadosCadastroExistentes =
      JSON.parse(localStorage.getItem('dadosCadastro')) || {};
    const novosDados = {
        ...
        <div className={styles.taskDivisaoUsuario}>
          ...
          <div className={styles.taskContent}>
            ...
            <div className={styles.taskLogin}>
              ...
              <button
                onClick={() => setModalSolicitarEmailAberto(true)}
                className={styles.btnAlterarSenhaAdmin}
              >
                Alterar Senha
              </button>
            </div>
            <button
              className={styles.btnSalvarAdmin}
              onClick={salvarDadosAdmin}
            >
              Salvar Alterações
            </button>
          </div>
                ...
                <div key={index} className={styles.nomeUsuarioCadastrado}>
                  ...
                  {idUsuarioSendoEditado === index && (
                    <div className={styles.camposEdicaoExpandidos}>
                      ...
                      <button
                        className={styles.salvarIndividual}
                        onClick={() => salvarEdicaoUsuario(index)}
                      >
                        Salvar Alterações
                      </button>
                    </div>
